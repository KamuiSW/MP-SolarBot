...
static bool extractWaypoints(const std::string &s, std::vector<Waypoint> &out) {
  ...
  while (true) {
    ...
    if (ox == std::string::npos)
      break;
    size_t cx = arr.find(':', ox);
    if (cx == std::string::npos)
      break;
    size_t sx = cx + 1;
    while (sx < arr.size() && (arr[sx] == ' '))
      sx++;
    size_t ex = sx;
    while (ex < arr.size() &&
           ((arr[ex] >= '0' && arr[ex] <= '9') || arr[ex] == '-'))
      ex++;

    size_t oy = arr.find("\"y\"", ex);
    if (oy == std::string::npos)
      break;
    size_t cy = arr.find(':', oy);
    if (cy == std::string::npos)
      break;
    size_t sy = cy + 1;
    while (sy < arr.size() && (arr[sy] == ' '))
      sy++;
    size_t ey = sy;
    while (ey < arr.size() &&
           ((arr[ey] >= '0' && arr[ey] <= '9') || arr[ey] == '-'))
      ey++;
...
static bool parseVisionResult(const std::string &s, bool &dirty,
                              double &score) {
  auto kd = s.find("\"dirty\"");
  auto ks = s.find("\"score\"");
  if (kd == std::string::npos || ks == std::string::npos)
    return false;

  auto cd = s.find(':', kd);
  auto cs = s.find(':', ks);
  if (cd == std::string::npos || cs == std::string::npos)
    return false;
...
bool runExecuteNavigation() {
  ...
  std::string js;
  if (!readFile(NAVIGATION_JSON_PATH, js))
    return false;

  std::vector<Waypoint> path;
  if (!extractWaypoints(js, path))
    return false;
  ...
  for (size_t i = 1; i < path.size(); i++) {
    ...
    while (curX != tx) {
      ...
      std::cout << "{\"type\":\"pose\", \"x\":" << curX << ", \"y\":" << curY
                << ", \"mode\":\"cleaning\"}" << std::endl;
      ...
      if (cellCounter % CHECK_EVERY_N_CELLS == 0) {
        ...
        if (runVisionOnce(dirty, score) && dirty) {
          if (!stainNearExisting(stainCache, curX, curY, DEDUPE_RADIUS_CELLS)) {
            appendStainJsonl(STAINS_JSONL_PATH, curX, curY, CELL_SIZE_CM,
                             score);
            stainCache.push_back({curX, curY, score});
            std::cout << "{\"type\":\"stain_found\", \"x\":" << curX
                      << ", \"y\":" << curY << ", \"score\":" << score << "}"
                      << std::endl;
          }
        }
        ...
        if (runVisionOnce(dirty, score) && dirty) {
          if (!stainNearExisting(stainCache, curX, curY, DEDUPE_RADIUS_CELLS)) {
            appendStainJsonl(STAINS_JSONL_PATH, curX, curY, CELL_SIZE_CM,
                             score);
            stainCache.push_back({curX, curY, score});
            std::cout << "[Stain] logged at (" << curX << "," << curY
                      << ") score=" << score << "\n";
          }
        }
      }
  ...
  buildStainVisitPlan(STAINS_JSONL_PATH, curX, curY, STAIN_PATH_JSON,
                      CELL_SIZE_CM);
